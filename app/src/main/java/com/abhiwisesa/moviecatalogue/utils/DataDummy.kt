package com.abhiwisesa.moviecatalogue.utils


object DataDummy {

//    fun generateDummyMovies(): List<MovieInList> {
//
//        val movies = ArrayList<MovieInList>()
//
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-11-03",
//                "Eternals",
//                "/bcCBq9N1EMo3daNIjWJ8kYvrQm6.jpg",
//                524434
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2022-01-13",
//                "Hotel Transylvania: Transformania",
//                "/teCy1egGQa0y8ULJvlrDHQKnxBL.jpg",
//                585083
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-11-24",
//                "Encanto",
//                "/4j0PNHkMr5ax3IA8tjtxcmPU3QT.jpg",
//                568124
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//        movies.add(
//            MovieInList(
//                "2021-12-15",
//                "Spider-Man: No Way Home",
//                "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//                634649
//            )
//        )
//
//        return movies
//    }
//
//    fun generateDummyTvShow(): List<ResultsItem> {
//
//        val movies = ArrayList<ResultsItem>()
//
//        movies.add(
//            ResultsItem(
//                "2019-06-16",
//                "/jtnfNzqZwN4E32FGGxx1YZaBWWf.jpg",
//                "Euphoria",
//                85552
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2022-01-13",
//                "/hE3LRZAY84fG19a18pzpkZERjTE.jpg",
//                "Peacemaker",
//                115036
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-12-29",
//                "/emuCvPY9ztbTM4ct4zqEQXraZli.jpg",
//                "The Book of Boba Fett",
//                115036
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "1995-10-23",
//                "/bpmLMZP3M1vLujPqHnOTnKVjRJY.jpg",
//                "Hollyoaks",
//                966
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2022-01-28",
//                "/ze4lhw0oLBHYmlM2KuZjBg0Sq6H.jpg",
//                "All of Us Are Dead",
//                99966
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//        movies.add(
//            ResultsItem(
//                "2021-02-22",
//                "/xhjoXm1WEvfQGYBGXKk8xk75z6s.jpg",
//                "Chain Reaction",
//                6455
//            )
//        )
//
//
//        return movies
//    }
//
//    fun generateDummyDetailMovie(): MovieResponse {
//        val genres: List<GenresItem> = emptyList()
//        var genre = GenresItem("Action", 28)
//        genres.toMutableList().add(genre)
//
//        genre = GenresItem("Adventure", 12)
//        genres.toMutableList().add(genre)
//
//        genre = GenresItem("Science Fiction", 878)
//        genres.toMutableList().add(genre)
//        return MovieResponse(
//            "en",
//            genres,
//            531241,
//            "Peter Parker is unmasked and no longer able to separate his normal life from the high-stakes of being a super-hero. When he asks for help from Doctor Strange the stakes become even more dangerous, forcing him to discover what it truly means to be Spider-Man.",
//            "Spider-Man: No Way Home",
//            "/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg",
//            "2021-12-15",
//            8.4,
//            "Released"
//        )
//    }
//
//    fun generateDummyDetailShow(): TvShowResponse {
//        val genres: List<GenresItem> = emptyList()
//        val genre = GenresItem("Drama", 18)
//        genres.toMutableList().add(genre)
//
//        return TvShowResponse(
//            "en",
//            genres,
//            85552,
//            "2019-06-16",
//            "A group of high school students navigate love and friendships in a world of drugs, sex, trauma, and social media.",
//            "/jtnfNzqZwN4E32FGGxx1YZaBWWf.jpg",
//            "Euphoria",
//            8.4
//        )
//    }
//
//    fun generateDummyMovieEntity(): MovieEntity {
//        val movie = generateDummyDetailMovie()
//        return MovieEntity(
//            movie.id,
//            movie.originalTitle,
//            movie.originalLanguage,
//            "Action, Adventure, Science Fiction",
//            movie.releaseDate,
//            movie.overview,
//            movie.posterPath,
//            movie.voteAverage,
//            true,
//            DateHelper.getCurrentDate()
//        )
//    }
//
//    fun generateListMovieEntity(): List<MovieEntity> {
//        val movie = generateDummyDetailMovie()
//        val movieList = ArrayList<MovieEntity>()
//        val movieEntity = MovieEntity(
//            movie.id,
//            movie.originalTitle,
//            movie.originalLanguage,
//            "Action, Adventure, Science Fiction",
//            movie.releaseDate,
//            movie.overview,
//            movie.posterPath,
//            movie.voteAverage,
//            true,
//            DateHelper.getCurrentDate()
//        )
//        movieList.add(movieEntity)
//        return movieList
//    }





}